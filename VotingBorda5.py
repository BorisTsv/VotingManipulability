...
for i in range(0, trials):
    ...
    if check is True:
        ...
        Sd = sorted(D)
        Se = sorted(E)
        if (Sb[0]>0) and (Sb[0]+Sb[1]>CoalBsize) and (Sb[0]+Sb[1]+Sb[2]>3*CoalBsize) and (Sb[0]+Sb[1]+Sb[2]+Sb[3]>6*CoalBsize):
            BmanipCheck = True
        if (Sc[0]>0) and (Sc[0]+Sc[1]>CoalCsize) and (Sc[0]+Sc[1]+Sc[2]>3*CoalCsize) and (Sc[0]+Sc[1]+Sc[2]+Sc[3]>6*CoalCsize):
            CmanipCheck = True
        if (Sd[0]>0) and (Sd[0]+Sd[1]>CoalDsize) and (Sd[0]+Sd[1]+Sd[2]>3*CoalDsize) and (Sd[0]+Sd[1]+Sd[2]+Sd[3]>6*CoalDsize):
            DmanipCheck = True
        if (Se[0]>0) and (Se[0]+Se[1]>CoalEsize) and (Se[0]+Se[1]+Se[2]>3*CoalEsize) and (Se[0]+Se[1]+Se[2]+Se[3]>6*CoalEsize):
            EmanipCheck = True
    if check is True:
        if (BmanipCheck is True and CmanipCheck is True and DmanipCheck is True and EmanipCheck is True):
            counter += 1
        else:
            NMcounter += 1
ans = 0.0
...
print(str(ans) + '%')
NMans = float(120 * NMcounter / 8000)
print(str(NMans) + '%')